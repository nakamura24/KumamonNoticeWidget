...
public class NoticeWidgetService extends RemoteViewsService {
	...
	public static NoticeWidgetCalendarEvent mCalendarEvent = new NoticeWidgetCalendarEvent();
	...
	class NoticeRemoteViewsFactory implements
		...
		public RemoteViews getViewAt(int position) {
			...
			RemoteViews remoteviews = new RemoteViews(
					mContext.getPackageName(), R.layout.widget_notice_item);
			if (mCalendarEvent.mEvents.get(position).all_day == 0) {
				...
				SimpleDateFormat sdf1 = new SimpleDateFormat("MM/d(EEE)",
						Locale.JAPANESE);
				remoteviews.setTextViewText(R.id.notice_date_TextView, sdf1
						.format(new Date(
								mCalendarEvent.mEvents.get(position).dtstart)));
			}
			remoteviews.setTextViewText(R.id.notice_title_TextView,
