...
public class NoticeWidgetCalendarEvent {
	private static final String TAG = "NoticeWidgetCalendarEvent";
	...
	public ArrayList<NoticeWidgetCalendarEventData> mEvents = new ArrayList<NoticeWidgetCalendarEventData>();
	...
	public void queryCalendar(Context context) {
		...
		try {
			...
			allday = configure.get(ConfigureAllDay, false);
			Period = configure.get(ConfigurePeriod, 14);
			// Run query
			Cursor cur = null;
			...
			String google_account = res.getString(R.string.notice_google_account);
			...
			if (cur.moveToNext()) {
				...
				Arrays.sort(temp, new NoticeWidgetDateComparator());
				mEvents.clear();
				for (int i = 0; i < temp.length; i++) {
					mEvents.add((NoticeWidgetCalendarEventData) temp[i]);
					Calendar cal_dtstart = Calendar.getInstance(); // 現在日時を取得
					cal_dtstart
							.setTimeInMillis(((NoticeWidgetCalendarEventData) temp[i]).dtstart);
					Log.d(TAG,
							"sorted - "
									+ cal_dtstart.getTime().toString()
									+ " "
									+ ((NoticeWidgetCalendarEventData) temp[i]).title
									+ " "
									+ String.valueOf(((NoticeWidgetCalendarEventData) temp[i]).all_day));
				}
			}
		} catch (Exception e) { // (2)
	...
	private void queryEvent(ContentResolver cr) {
		...
		try {
			...
			while (eventCursor.moveToNext()) {
				...
				NoticeWidgetCalendarEventData event = new NoticeWidgetCalendarEventData();
